...
class PermissionViewMode extends JSDialogComponent {
	...
	protected setupContainer(parentContainer?: HTMLElement): void {
		...
		this.viewModeDropdown = window.L.DomUtil.create(
			'div',
			'viewModeDropdown notebookbar hidden',
			this.viewModeContainer,
		);
	...
	private _getMenuButtonJSON(text: string) {
		return {
			type: 'menubutton',
			...
			image: false,
			menu: [
				{ id: 'view', text: _('Viewing Mode') },
				{ id: 'edit', text: _('Editing Mode') },
			],
		};
	}
	...
	private _setupDropdown(): void {
		this.createBuilder();
		const data = [this._getMenuButtonJSON(_('Viewing'))];
	...
	updateReadonlyIndicator(): void {
		...
		if (showReadonly) {
			window.L.DomUtil.removeClass(this.readonlyIndicator, 'hidden');
		} else {
			window.L.DomUtil.addClass(this.readonlyIndicator, 'hidden');
		}

		if (showDropdown) {
			window.L.DomUtil.removeClass(this.viewModeDropdown, 'hidden');
			this.updateDropdownText(dropdownText);
		} else {
			window.L.DomUtil.addClass(this.viewModeDropdown, 'hidden');
		}
	}

	updateDropdownText(text: string) {
		this.builder?.updateWidget(
			this.viewModeDropdown,
			this._getMenuButtonJSON(text),
		);
	}
}
